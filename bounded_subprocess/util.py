...
class BoundedSubprocessState:
    ...
    def try_read(self) -> bool:
        ...
        if this_stdout_read is not None and self.stdout_bytes_read < self.max_output_size:
            self.stdout_saved_bytes.append(this_stdout_read)
            self.stdout_bytes_read += len(this_stdout_read)
        if this_stderr_read is not None and self.stderr_bytes_read < self.max_output_size:
